(ns courierops.operation
  "OperationActor -- one courier operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (CourierOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Courier Governor (:govern) and the rollout phase
  ...
  One graph run = one courier operation (intake -> advise -> govern ->
  ...
  decision to a human operator (courier dispatcher/operator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:delivery/dispatch`/`:delivery/settle` ALWAYS reach
  this node when the governor is clean -- see `courierops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [courierops.courieropsllm :as courieropsllm]
            [courierops.governor :as governor]
...
(defn build
  ...
  `courierops.store/Store`).
  opts:
    :advisor      -- a `courierops.courieropsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (courieropsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CourierOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (courieropsllm/-advise advisor store request)]
            {:proposal p :audit [(courieropsllm/trace request p)]})))

      ;; Courier Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
